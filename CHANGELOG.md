...
### 개선 — REST API 응답 헤더 UX & 테이블 레이아웃

**응답 헤더 테이블**
- 컬럼 드래그 리사이저 추가 — Name / Value 비율 20~80% 범위 내 자유 조절
- 외곽선 상·하만 표시 (좌·우 제거), 헤더 행 구분선 2px (본문 행 1px)
- Name · Value 데이터 좌측 정렬 수정
- Value 셀 클릭 시 전체 값 팝오버 표시 + 복사 버튼

**요청 Params / Headers 테이블**
- 컬럼 드래그 리사이저 추가 — Key / Value 비율 조절
- 외곽선 상·하만 표시, 헤더 행 구분선 2px

**Cookies / Assertions 테이블**
- 동일한 리사이저 및 외곽선 스타일 적용
- Cookies: Name · Value 컬럼 각각 리사이저
- Assertions: 유형 · 경로 · 조건 컬럼 각각 리사이저

**레이아웃**
- 요청 영역 / 응답 영역 사이 수직 드래그 리사이저 추가 — 응답 첫 수신 시 50:50 초기 분할, 각 영역 최소 120px
- Auth 탭이 응답 영역을 침범하는 오버플로 버그 수정
- Auth 탭 콘텐츠 스크롤 지원

**클립보드**
- `navigator.clipboard` 미동작 문제 수정 — `vscode.env.clipboard` 경유 postMessage 방식으로 변경 (export curl/fetch 등 전체 복사 기능 정상화)
